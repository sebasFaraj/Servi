...
const LocationMap = ({ 
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // For demo purposes, generate coordinates for the entered address
    const lat = 40.7128 + (Math.random() - 0.5) * 0.01;
    const lng = -74.0060 + (Math.random() - 0.5) * 0.01;
    onLocationSelect({ lat, lng, address: searchQuery });
  };
      ...
      <div className="h-full flex flex-col">
        <div className="bg-teal-600 dark:bg-teal-800 text-white p-4">
          <div className="flex items-center mb-4">
            ...
            <h2 className="text-xl font-semibold ml-4">
              Select {type ? (type === 'pickup' ? 'Pickup' : 'Drop-off') : label}
            </h2>
          </div>
        ...
        <div className="flex-1 relative">
          ...
          >
            ...
            {type === 'pickup' && selectedLocations.dropoff && (
              <LocationPin
                type="dropoff"
            ...
            {type === 'dropoff' && selectedLocations.pickup && (
              <LocationPin
                type="pickup"
